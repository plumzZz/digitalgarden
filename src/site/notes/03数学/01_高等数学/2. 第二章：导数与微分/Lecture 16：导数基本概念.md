---
{"title":"Lecture 16：导数基本概念","tags":["数学","导数"],"categories":null,"date":"2023-12-13","dg-publish":true,"permalink":"/03数学/01_高等数学/2. 第二章：导数与微分/Lecture 16：导数基本概念/","dgPassFrontmatter":true}
---
...
![Pasted image 20250606172914.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020250606172914.png)
...
+ 图示：
	+ ![Pasted image 20240314175211.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020240314175211.png)
...
+ s 为 $s(t)$
+ 距离与速度关系 
	+ ![Pasted image 20240324194344.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020240324194344.png)
+ 变化量：
	+ ![Pasted image 20240324194646.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020240324194646.png)
+ 对一点附近的变化率推导：$\begin{gathered}\text{Derivative}\\\frac{ds}{dt}(t)=\underbrace{\frac{s(t+dt)-s(t)}{dt}}_{dt\to0}\end{gathered}$
+ 假设在 $t=2$ 的点，求这一点的切线：$\begin{aligned}\frac{ds}{dt}(2)=\frac{\left(2+dt\right)^3-\left(2\right)^3}{dt}\end{aligned}$
+ 此时将 $(2+dt)^3$ 展开，得到：$\begin{aligned}\frac{2^3+3(2)^2dt+3(2)(dt)^2+(dt)^3-2^3}{dt}\end{aligned}$
+ 将其除以 $dt$ 后，得到：$3(2)^2+3(2)(dt)+(dt)^2$
...
### 导函数与奇偶性
![Pasted image 20250606174305.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020250606174305.png)
![Pasted image 20250606174245.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020250606174245.png)
...
### 周期函数的导数周期性分析
*要具体一点过程去发现*
对于以 $T$ 为周期的周期函数 $f(x)$（本题中 $T=2$），其导函数同样具有周期性。以下是严格证明：

#### 1. **一阶导数的周期性**
设 $f(x+2) = f(x)$ 对定义域内所有 $x$ 成立。  
对等式两边求导：
$$
...
\boxed{f'(x+2) = f'(x)}
$$
$\therefore$ 一阶导函数 $f'(x)$ 也以 $2$ 为周期。

---

#### 2. **二阶导数的周期性**
对一阶导函数 $f'(x+2) = f'(x)$ 两边再次求导：
$$
\frac{d}{dx}[f'(x+2)] = \frac{d}{dx}[f'(x)]
$$
应用链式法则：
...
\boxed{f''(x+2) = f''(x)}
$$
$\therefore$ 二阶导函数 $f''(x)$ 同样以 $2$ 为周期。

---

### 本题具体应用
在题目中：
- **$N = f'\left(\frac{3}{2}\right)$ 的计算**：  
  利用周期性 $f'(x+2) = f'(x)$：  
  $$
  f'\left(\frac{3}{2}\right) = f'\left(\frac{3}{2} - 2\right) = f'\left(-\frac{1}{2}\right)
  $$
  再结合 $f(x)$ 是奇函数（其导函数 $f'$ 是偶函数）：  
  $$
  f'\left(-\frac{1}{2}\right) = f'\left(\frac{1}{2}\right) > 0 \quad (\text{给定条件})
  $$

- **$K = f''(0)$ 的计算**：  
  利用周期性 $f''(x+2) = f''(x)$：  
  $$
  f''(0) = f''(0 + 2) = f''(2) = f''(0) \quad \text{(自洽)}
  $$
  结合 $f(x)$ 是奇函数（其二次导 $f''$ 是奇函数）：  
  $$
  f''(-x) = -f''(x) \implies f''(0) = -f''(0) \implies 2f''(0) = 0 \implies \boxed{f''(0) = 0}
  $$
...
#### 奇偶函数求导一次，奇偶性就互换一次
![Pasted image 20250606180438.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020250606180438.png) ![Pasted image 20250606180724.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020250606180724.png)

**重要结论**
使不可导变成可导
![Pasted image 20250607153554.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020250607153554.png)
*证明过程*
	![Pasted image 20250607153647.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020250607153647.png)
![Pasted image 20250607153903.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020250607153903.png)
...
![Pasted image 20250607161051.png](/img/user/01%E9%99%84%E4%BB%B6/Pasted%20image%2020250607161051.png)


切线存在的时候导数不一定存在; 导数存在的时候切线一点存在。